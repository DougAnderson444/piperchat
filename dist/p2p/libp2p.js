...
export async function startLibp2p(bootstrapNodes = [WEBRTC_BOOTSTRAP_NODE]) {
    ...
    let peerDiscovery = bootstrapNodes.length > 0
        ? [
            bootstrap({
                list: bootstrapNodes,
...
export const setWebRTCRelayAddress = (maddrs, peerId) => {
    maddrs.forEach((maddr) => {
        if (maddr.protoCodes().includes(CIRCUIT_RELAY_CODE)) {
            ...
            copyToClipboard(document.querySelector('#copy'));
        }
    });
};
export function setCircuit(value) {
    document.querySelector('#circuit').value = value;
}
...
export const publishMessage = (libp2p) => async (message) => {
    try {
